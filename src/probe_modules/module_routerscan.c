		...
		__attribute__((unused)) uint32_t len, fieldset_t *fs)
{
	...
	//use the new struct icmp_hdr defined in ./routermessage.h 
	//which define the time-exceed message IP-ICMP-(IP-ICMP)
	struct icmphdr_ip *icmp_hdr = (struct icmphdr_ip *) ((char *) ip_hdr + 4 * ip_hdr-> ip_hl);
	//struct icmphdr_ip *icmp_hdr = (struct icmphdr_ip *) ((char *) ip_hdr + 1);

	fs_add_uint64(fs, "icmptype", icmp_hdr->icmp_type);
	fs_add_uint64(fs, "icmpcode", icmp_hdr->icmp_code);
	fs_add_uint64(fs, "sendttl", zconf.max_ttl);
	fs_add_string(fs, "orgdst", make_ip_str(icmp_hdr->ip_dst), 0);
...
fielddef_t routerscan_fields[] = {
	{.name="icmptype", .type="int", .desc="icmp message type"},
	{.name="icmpcode", .type="int", .desc="icmp message code"},
	{.name="sendttl", .type="int", .desc="max ttl"},
	{.name="orgdst", .type="string", .desc="orginal destination address"},
	{.name="type", .type="string", .desc="desc of icmp type"},
