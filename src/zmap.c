...
static void json_metadata(FILE *file)
{
	...
	json_object_object_add(obj, "max-ttl",
			json_object_new_int(zconf.max_ttl));
...
{
	log_debug("zmap", "Max TTL is set to (%d)", zconf.max_ttl);
	if (zconf.iface == NULL) {
...
int main(int argc, char *argv[])
{
	...
	//ttl conf support for routerscan
	if(!args.max_ttl_given){
		zconf.max_ttl = MAXTTL;
	} else {
		enforce_range("max-ttl", args.max_ttl_arg, 0, MAXTTL);
		zconf.max_ttl = args.max_ttl_arg;
	}
